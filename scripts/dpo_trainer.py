...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        with PartialState().local_main_process_first():
            ...
            train_dataset = train_dataset.map(self.tokenize_row, num_proc=args.dataset_num_proc,fn_kwargs=fn_kwargs)
            if eval_dataset is not None:
                eval_dataset = eval_dataset.map(self.tokenize_row, num_proc=args.dataset_num_proc,fn_kwargs=fn_kwargs)
    ...
    def concatenated_forward_ref(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]]):
    ...
    def concatenated_forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]]):
        ...
        return (chosen_logps, rejected_logps, chosen_logits, rejected_logits,extra)
    ...
    def get_batch_loss_metrics(
                ...
                if self.ref_model is None:
                    with self.null_ref_context():
                        (
                            reference_chosen_logps,
                            reference_rejected_logps,
                        ) = self.concatenated_forward_ref(self.model, batch)
                else:
                    (
                        reference_chosen_logps,
                        reference_rejected_logps,
                    ) = self.concatenated_forward_ref(self.ref_model, batch)
        ...
        if self.args.sft_weight > 0:
            ...
            losses = losses + extra_tensors["nll_loss"] * self.args.sft_weight
            metrics[f"{prefix}nll_loss"] = extra_tensors["nll_loss"].detach().mean().cpu()
